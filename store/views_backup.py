...
('ᱵᱟᱺᱰᱭᱟᱹ ᱡᱷᱮᱴ','BADYA JHET',True), ('ᱟᱥᱟᱲ','ASAR',False),
...
def ensure_months():
    for i,(o,e,x) in enumerate(MONTH_DEFAULTS,1):
        LunarMonth.objects.get_or_create(order=i, defaults={'olchiki_name':o,'english_name':e,'is_extra':x,'active':True})
...
def staff_required(request): return request.user.is_authenticated and request.user.is_staff
...
def upload_image(request):
    ...
    if ext not in allowed:
        return JsonResponse({'error':'Allowed image types: JPG, JPEG, PNG, WEBP, GIF, SVG'}, status=400)
    if f.size > 5*1024*1024:
        return JsonResponse({'error':'Image must be 5 MB or smaller'}, status=400)
    fs=FileSystemStorage(location=settings.MEDIA_ROOT / 'calendar_images', base_url=settings.MEDIA_URL + 'calendar_images/')
...
def bootstrap(request):
    ...
    return JsonResponse({
        ...
        'newMoons':[{'id':x.id,'date':x.date.isoformat(),'lunar_year':x.lunar_year,'month_order':x.month_order,'note':x.note} for x in NewMoon.objects.all()],
...
def calendar_month(request):
    ...
    except (KeyError,ValueError): return JsonResponse({'error':'year and month required'}, status=400)
    start=date(y,m,1); end=date(y+1,1,1) if m==12 else date(y,m+1,1)
    return JsonResponse({
        'history':{x.gregorian.isoformat():hist_json(x) for x in HistoricalDate.objects.filter(gregorian__gte=start,gregorian__lt=end)},
        'newMoons':[{'id':x.id,'date':x.date.isoformat(),'lunar_year':x.lunar_year,'month_order':x.month_order} for x in NewMoon.objects.filter(date__gte=start-timedelta(days=40),date__lt=end+timedelta(days=40))],
        'events':[event_json(x) for x in LunarEvent.objects.filter(active=True)],
        'solarEvents':[solar_json(x) for x in SolarEvent.objects.filter(active=True,date__gte=start,date__lt=end)],
        'ads':[ad_json(x) for x in Advertisement.objects.filter(active=True)]
    })
...
def data_api(request, kind):
    if not request.user.is_staff: return JsonResponse({'error':'Admin login required'}, status=403)
    try: body=json.loads(request.body or '{}')
    except json.JSONDecodeError: return JsonResponse({'error':'Invalid JSON'}, status=400)
    model_map={'month':LunarMonth,'moon':NewMoon,'history':HistoricalDate,'event':LunarEvent,'solar':SolarEvent,'ad':Advertisement,'setting':SiteSetting}
    Model=model_map.get(kind)
    if not Model: return JsonResponse({'error':'Unknown resource'}, status=404)
    obj=Model.objects.filter(pk=body.get('id')).first() if body.get('id') else None
    if request.method=='DELETE':
        if not obj: return JsonResponse({'error':'Not found'}, status=404)
        obj.delete(); return JsonResponse({'ok':True})
    if kind=='month':
        obj=obj or Model(); obj.order=int(body['order']); obj.olchiki_name=body.get('olchiki_name',''); obj.english_name=body.get('english_name',''); obj.is_extra=bool(body.get('is_extra')); obj.active=True
    elif kind=='moon':
        obj=obj or Model(); obj.date=body['date']; obj.lunar_year=body.get('lunar_year') or None; obj.month_order=body.get('month_order') or None; obj.note=body.get('note','')
    elif kind=='history':
        obj=obj or Model(); obj.gregorian=body['date']
        ...
        obj=obj or Model(); obj.slot=body['slot']; obj.business_name=body['business_name']; obj.image=body.get('image',''); obj.target_url=body.get('target_url',''); obj.start_date=body.get('start_date') or None; obj.end_date=body.get('end_date') or None; obj.priority=int(body.get('priority',0)); obj.active=True
    elif kind=='setting':
        obj, _ = Model.objects.get_or_create(key=body['key']); obj.value=body.get('value','')
    try: obj.save()
...
def import_csv(request, kind):
    if not request.user.is_staff: return JsonResponse({'error':'Admin login required'},status=403)
    if 'file' not in request.FILES: return JsonResponse({'error':'CSV file required'},status=400)
    raw=request.FILES['file'].read().decode('utf-8-sig'); reader=csv.DictReader(io.StringIO(raw)); count=0
    try:
        for r in reader:
            if kind=='history':
                HistoricalDate.objects.update_or_create(gregorian=r['gregorian'],defaults={'bengali_day':r.get('bengali_day') or None,'bengali_month':r.get('bengali_month',''),'bengali_year':r.get('bengali_year') or None,'santali_day':r.get('santali_day') or None,'santali_month':r.get('santali_month',''),'santali_year':r.get('santali_year') or None,'note':r.get('note','')}); count+=1
            elif kind=='moon':
...
def export_csv(request, kind):
    ...
    fields=[]; rows=[]
    if kind=='history':
        fields=['gregorian','bengali_day','bengali_month','bengali_year','santali_day','santali_month','santali_year','note']; rows=[[x.gregorian,x.bengali_day,x.bengali_month,x.bengali_year,x.santali_day,x.santali_month,x.santali_year,x.note] for x in HistoricalDate.objects.all()]
    elif kind=='moon':
